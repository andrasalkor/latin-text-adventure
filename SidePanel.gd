...
func handle_room_changed(new_room):
	room_name.text = new_room.into_preposition.capitalize() + " " + new_room.get_into_preposition_case(new_room.into_preposition)
	...
	# var npc_string = hide_empty_label(new_room.get_npc_description()
	if npc_string == "":
		npc_label.hide()
	else:
		npc_label.show()
